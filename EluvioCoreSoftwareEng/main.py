...
# Function to find the longest common substring given two strings
def findLongestSubstring(strOne, strTwo):
    ...
    if maxLength > 0:
        outputString = strOne[start : start + maxLength]

    return outputString

# Function to find all files and offsets that contain the given substring -> returns a list of lists
...
def main():
    ...
    for file in os.listdir("data"):
        bytes[file] = Path("./data/" + file).read_bytes()
    ...
    for file_pair in combinations(bytes, 2):
        ...
        substring = findLongestSubstring(fileOne, fileTwo)
